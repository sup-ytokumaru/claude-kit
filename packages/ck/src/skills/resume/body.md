...
handoff ファイルが複数ある場合は最新を仮選択して進み、仮選択であることと他候補の一覧を最終報告に明記する。Step 4 の「続けますか？」の確認は行わず、展開したコンテキストと推奨する次アクションの提示で停止する（承認なしに実装アクションを開始しない）。
...
handoff の内容を展開し、Step 4 の再開確認を提示した時点で完了。承認後は Pending work に記載されたタスクのスキル（`/implement`・`/plan` 等）に戻って続ける。
